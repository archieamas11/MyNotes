...
<div class="" id="addNoteForm" style="display: none;">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="data-item">
                            ...
                            $stmt = $conn->prepare("SELECT * FROM `tbl_notes` WHERE is_archived = 1");
                            $stmt->execute();
    ...
    <?php
        ...
        <button onclick="unarchiveNote(<?php echo $noteID; ?>)" title="unarchiveNote"><i class="fas fa-archive"></i></button>
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4) {
                if (this.status == 200) {
                    ...
                    renderSearchResults(searchResults); // Call the renderSearchResults function with the results
                } else {
...
function renderSearchResults(results) {
    var listGroup = document.querySelector('.list-group'); // Assuming this is the correct element where you want to display the search results

    // Clear existing content in the listGroup
    listGroup.innerHTML = '';
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (this.responseText.trim() === 'success') {
                // Remove the unarchived note from the DOM
                var noteElement = document.getElementById('note_' + noteID);
                if (noteElement) {
                    noteElement.parentNode.removeChild(noteElement);
                }
                ...
                // Display an error message if unarchiving failed
                alert('Failed to unarchive note. Please try again.');
            }
        }
    };
    xhttp.open("GET", "unarchive_note.php?note_id=" + noteID, true);
    xhttp.send();
}
